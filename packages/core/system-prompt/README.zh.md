...
`decidePromptEvolution(candidate, gate)` 使用匹配的评估种子、训练集提升、无回退的留出集结果、已完成的泄漏检查和最大字符增长比率，评估只含摘要的单分支提示词候选。每个模型的迁移分数会作为证据保留，但不会隐式覆盖主要留出集门控。拒绝的决策包含机器可读原因。`rollbackPromptEvolution(decision, reason)` 记录恢复到基础摘要，并且只接受已经通过的决策。这些帮助函数不会公开提示词正文，也不会修改、注册、部署或激活提示词。
