...
在步骤内，独占调用形成屏障；并行安全调用使用有界滚动池，并在启动前重新分类。工具可声明 `schedule: 'after-batch'`，以便在同一 assistant 步骤中的所有普通调用完成后再运行；即使分发顺序发生变化，调度器仍按模型顺序保留持久调用、结果及结果上下文。工具可声明 `coalesceDuplicates: true`，使该步骤中后续相同名称且规范参数相同的调用跳过分发，并收到一份指向首次调用的成功占位结果；未标记的工具仍会执行每一次调用。只有分发和调用主体的执行会发生重叠。中止会阻止启动新的调用，等待已启动调用的结果处理完毕，并保留其完成终结后的结果上下文，不区分取消原因。内部调度器故障会停止新的分发，等待已启动的分发，然后在不虚构工具结果的情况下到达轮次错误边界。
