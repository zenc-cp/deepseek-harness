...
- `ctx.tools.executionMode(exec)` returns `parallel` only when the visible definition's `isConcurrencySafe(exec.arguments)` classifier returns exactly `true`; unknown, hidden, undeclared, invalid, or throwing classifications are exclusive. A definition may additionally declare `schedule: 'after-batch'`, which the top-level agent-loop scheduler uses to defer that call until ordinary calls from the same assistant step settle without exposing the metadata to the model.
