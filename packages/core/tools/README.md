...
- `ctx.tools.executionMode(exec)` returns `parallel` only when the visible definition's `isConcurrencySafe(exec.arguments)` classifier returns exactly `true`; unknown, hidden, undeclared, invalid, or throwing classifications are exclusive. A definition may additionally declare `schedule: 'after-batch'`, which the top-level agent-loop scheduler uses to defer that call until ordinary calls from the same assistant step settle without exposing the metadata to the model. `ctx.tools.shouldCoalesceDuplicates(exec)` is true only for an exact `coalesceDuplicates: true` declaration; unmarked, hidden, or unknown tools keep every call. The scheduler then skips later identical name-plus-canonical-argument calls in that assistant step, still recording one durable call/result pair per model-issued call.
...
- **Concurrency policy is not an event gate** — `executionMode()` and `shouldCoalesceDuplicates()` read the resolved tool definition directly; plugins can only declare classifiers or coalesce flags on definitions they own.
