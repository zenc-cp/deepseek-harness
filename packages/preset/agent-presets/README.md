...
- `ctx.agentPresets.manifest(id?): Promise<PresetManifest>` A versioned, deterministic read-only snapshot of the resolved standing composition: enabled/disabled Loader rows, model-visible tools, prompt-section names, and services provided by the preset subtree. It starts no agent, session, or turn; omits absolute paths and prompt text and replaces credential-shaped config leaves with `[redacted]`. `diffPresetManifests(before, after)` returns stable path-addressed changes without consulting live runtime state.
