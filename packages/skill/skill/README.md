...
`auditSkillAuthoring(markdown)` classifies a skill body as simple or complex. Simple skills (no Phase heading) stay concise. A Phase heading fail-closes unless every phase names Input, Actions, Output, Verify, and On failure, and the body states a terminal completion condition. `draftComplexSkillTemplate(name, phaseNames)` returns SKILL.md text that satisfies that audit. Neither helper writes, installs, or registers a skill.
