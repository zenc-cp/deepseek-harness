...
原始事件突发还会触发一次仅检查元数据的恢复扫描，范围为监视根目录的直属技能候选项。每个根最多保留一个待执行定时器或活动扫描；扫描期间到达的事件合并为一次后续检查。延迟使用配置的稳定阈值（至少 10 毫秒）；这是事件驱动恢复，而非周期轮询。指纹排除 atime/ctime，避免读取引发失效循环。释放时取消待执行任务并等待活动扫描结束。此回退机制无法发现同时保留文件标识、大小、mtime 和 mode 的内容修改，扫描成本随根目录直属条目数增长。
