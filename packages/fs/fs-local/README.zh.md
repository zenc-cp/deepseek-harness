...
完整或流式读取普通 UTF-8 文件，读取有界的原始字节，并按稳定名称顺序列出一层目录。原子地创建、替换或编辑文本。同一后端实例的变更按目标串行化。省略版本防护即无条件变更；提供防护则拒绝已变化的观察记录。

内容绑定的读取版本组合身份、大小、写入时间、基本权限与原始字节的完整 SHA-256。读取引起的访问时间/ctime 变化不会使其失效。旧式元数据 token 保持严格检查。变更结果绑定实际写入的字节，而不是后续写入方产生的未见字节。`stat`、`lstat` 和目录列出从不对内容计算哈希。
...
| [`src/snapshot.ts`](src/snapshot.ts) | 内容绑定的读取与摘要验证 |
...
每次编辑在字面量匹配前读取并验证现有字节，因此陈旧编辑报告 `FS_STALE_VERSION`，而不是误导性的无匹配。内容防护的编辑直接解码已验证的字节，无需再次读取内容，再以 LF 规范化执行字面量替换、恢复主要行尾风格，并走原子发布路径。两个变更路径都在每目标锁内，于暂存完成后、发布紧前再次检查内容防护。
...
- [内容绑定的版本](../../../.agents/notes/implemented/bug-fix/2026-09-05-content-bound-file-revisions.zh.md)：读取引起的元数据变化为何需要原始字节版本。
...
- **内容防护要求完整扫描**：快照观察需要一次整文件扫描；内容防护的变更需要两次。最后一次扫描不保留内容。如果存储层在重写时保留所有版本字段，旧式元数据 token 仍可能漏掉变化。
...
- **每目标锁仅限进程内**：带防护的创建跨进程使用原子且不替换的发布方式。替换没有内核级比较并替换原语：在最终验证到发布的窗口内，外部写入方的内容仍可能被覆盖。此锁从不串行化外部写入方。
