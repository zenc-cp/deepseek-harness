...
工具就是执行器；策略是事件门禁。工具不注入策略服务，也不检查任何缓存——每次变更都通过 `ctx.waterfall` 向单一意图槽位请求防护，每个操作只在成功后发出 `fs/observed`。读取调用一次提供方 `stat` 以完成元数据路由。若支持内容绑定读取，观察记录使用其完成版本，否则使用元数据版本。窗口化文本读取仍在消费完整流之后才记录版本；失败或未完成的读取不记录版本。变更工具不调用 `stat`：意图槽位提供防护，提供方在锁内重新检查。
